...
class TypingGame:
    ...
    def recite_words(self):
        ...
        for word in self.words:
            if word.matched and not hasattr(word, 'recited'):
                wav_file_path = os.path.join('recite', word.word + '.wav')
                if not os.path.exists(wav_file_path):
                    with wave.open(wav_file_path, "wb") as wav_file:
                        self.voice.synthesize_wav(word.word, wav_file, syn_config=self.syn_config)
                    word.recited = True
                playsound(wav_file_path, block=True)
